...
@section('content')
                ...
                <div class="breadcrumb-wrapper">
                    <nav>
                        <ol class="base-breadcrumb breadcrumb-three">
                            ...
                            <li class="active"><span><i class="lni lni-angle-double-right"></i></span>Artist Type</li>
                        </ol>
                    </nav>
                </div>
            </div>
    ...
    <!-- ArtistType Modal start -->
    <div class="follow-up-modal">
        <div class="modal fade" id="ArtistTypeModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content card-style text-center">
                    ...
                    <form id="CreateUpdateModal" method="POST">
    ...
    <script>
        $(document).ready(function() {
            ...
            function sendOrderToServer() {
                ...
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{ route('artist.type.rendaring') }}",
                    data: {
                        order: order,
                        _token: token
                    },
        ...
        $(document).ready(function() {
            $('#addBtn').on('click', function() {
                $("#CreateUpdateModal")[0].reset();
                $("#id").val('');
                $('#ArtistTypeModal').modal('show');
            });
        });


        // Submit Form
        $("#CreateUpdateModal").on('submit', function(e) {
            e.preventDefault();

            $.ajax({
                type: "POST",
                url: "{{ route('create.update.artist.type') }}",
                headers: {
                ...
                success: function(resp) {
                    ...
                    if (resp.success === true) {
                        ...
                        $('#ArtistTypeModal').modal('hide');
                        $("#VreateUpdateModal")[0].reset();
                    } else if (resp.errors) {
                        console.log(resp.errors);
                        // Show error messages on the form
                        $('#title').next('.valid-feedback').text(resp.errors.title);
                    } else {
        ...
        function ShowUpdateModal(id) {
            let url = '{{ route('artist.type.get', ':id') }}';
            $.ajax({
                type: "GET",
        ...
        function statusChange(id) {
            let url = '{{ route('artist.type.status', ':id') }}';
            $.ajax({
                type: "GET",
                url: url.replace(':id', id),
                success: function(resp) {
        ...
        function deleteItem(id) {
            let url = '{{ route('artist.type.destroy', ':id') }}';
